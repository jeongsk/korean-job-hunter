#!/usr/bin/env node
// Test: company extraction via "last segment before 경력" fallback
// Hypothesis: After pre-segmentation, the company is the last word/token before "경력"

const testCases = [
  { raw: "글로벌 패션 파트너 사업_CTO(Chief Technology Officer)페칭경력 8년 이상합격보상금 100만원", expect: { company: "페칭", title_includes: "CTO" } },
  ...
  // Known companies should still work
  ...
  // Pure English company
  { raw: "React DeveloperVingle경력 3년 이상합격보상금 70만원", expect: { company: "Vingle", title_includes: "React" } },
];

function escapeRegExp(s) { return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }

// NEW: Extract company as last Korean/Hangul-containing token before 경력
function extractCompanyBeforeExperience(text, preSegText) {
  // After pre-segmentation, text looks like: "[title][company] 경력 ..."
  // Find the segment just before "경력"
  const beforeExp = preSegText.split(/\s*경력/)[0];
  if (!beforeExp) return null;

  // The company is the last meaningful token
  // Strategy: split into tokens, find the last Korean-containing token or parenthesized group
  const tokens = beforeExp.trim().split(/\s+/);

  // Try from end: find last token containing Korean chars or being inside/after parentheses
  for (let i = tokens.length - 1; i >= 0; i--) {
    ...
    if (/[가-힣]/.test(tok)) {
      // Could be a company like "페칭", "화해글로벌", "제이앤피메디"
      return tok;
    }

    // Check if token is parenthesized like (lululab) or (JNPMEDI) — part of company name
    if (/^\(/.test(tok) && /\)$/.test(tok)) {
      // This is a parenthetical — the company is the token before this
      if (i > 0) {
        const prev = tokens[i - 1];
        if (/[가-힣]/.test(prev)) {
          return prev + tok; // e.g., "룰루랩(lululab)"
        }
      }
      continue;
    }

    // EXP-038: Pure English — check for camelCase company boundary
    // e.g., "DeveloperVingle" → company is "Vingle"
    if (/^[A-Za-z]+$/.test(tok) && i === tokens.length - 1) {
      const ccMatch = tok.match(/([a-z])([A-Z][a-z]+)$/);
      if (ccMatch) {
        return tok.substring(ccMatch.index + 1); // "Vingle"
      }
    }
  }
...
function parseWantedJob(raw) {
  ...
  // Save pre-segmented text for company extraction
  const preSegText = t;
  ...
  // NEW: Last-segment-before-경력 fallback
  if (!cm) {
    cm = extractCompanyBeforeExperience(t, preSegText);
    if (cm) {
      // Clean parenthetical suffix from display name (keep core name)
      const displayName = cm.replace(/\(.*?\)$/, '');
      // Remove from title text
      t = t.replace(new RegExp(escapeRegExp(cm), 'g'), ' ').trim();
      // If the parenthetical part remains, also remove it
      t = t.replace(/\([^)]*\)/g, m => {
        // Only remove if it looks like a company English name
        if (/^[A-Za-z]/.test(m.slice(1))) return ' ';
        return m;
      }).trim();
      cm = displayName;
    }
