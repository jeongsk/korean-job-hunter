...
const CITIES = '(서울|경기|부산|대전|인천|광주|대구|울산|판교|강남|영등포|송파|성수|역삼|잠실|마포|용산|구로|분당|일산|평촌|수원|이천|세종|여의도|신촌|홍대|건대|동탄|청주|천안|양재|논현|신사|삼성|방배|광화문|을지로|종로|시흥|안양|안산|평택|파주|김포|창원|포항)';
...
function parseWantedJob(raw) {
  ...
  if (/재택\s*불가|원격\s*불가|리모트\s*불가|remote\s*불가/i.test(t)) {
    ...
    r.work_type = 'remote';
  } else if (/재택|remote|리모트|원격/i.test(t)) {
    r.work_type = 'remote';
  } else {
    r.work_type = 'onsite';
  }
