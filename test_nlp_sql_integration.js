...
function parseKoreanQuery(input) {
  ...
  const keywordMatch = text.match(/(백엔드|프론트엔드|프론트|데이터|AI|머신러닝|DevOps|devops|인프라|모바일|안드로이드|iOS|ios)/);
  ...
  if (/(마감임박|곧마감|마감 임박)/.test(text)) {
    ...
    filters.push("j.deadline IS NOT NULL AND ROUND(julianday(j.deadline) - julianday('now'), 4) BETWEEN -0.5 AND 0.5");
    consumedWords.add('오늘 마감');
  } else if (/내일 마감/.test(text)) {
    filters.push("j.deadline IS NOT NULL AND ROUND(julianday(j.deadline) - julianday('now'), 4) BETWEEN 0.5 AND 1.5");
...
const jobs = [
  { id: 'j1', source: 'wanted', title: '백엔드 엔지니어', company: '카카오', location: '서울 영등포구', work_type: 'hybrid', experience: '경력 5년 이상', salary: '8000~12000만원', salary_min: 8000, salary_max: 12000, deadline: nextWeek },
  { id: 'j2', source: 'wanted', title: '프론트엔드 개발자', company: '네이버', location: '경기 성남시', work_type: 'remote', experience: '경력 3년 이상', salary: '6000~9000만원', salary_min: 6000, salary_max: 9000, deadline: tomorrow },
  { id: 'j3', source: 'jobkorea', title: '데이터 엔지니어', company: '토스', location: '서울 강남구', work_type: 'onsite', experience: '경력 2년 이상', salary: '5000~8000만원', salary_min: 5000, salary_max: 8000, deadline: today },
  { id: 'j4', source: 'linkedin', title: 'DevOps Engineer', company: '라인', location: '서울 영등포구', work_type: 'hybrid', experience: '경력 무관', salary: null, salary_min: null, salary_max: null, deadline: null },
  { id: 'j5', source: 'wanted', title: 'iOS 개발자', company: '카카오뱅크', location: '서울 강남구', work_type: 'onsite', experience: '신입 가능', salary: '4000~6000만원', salary_min: 4000, salary_max: 6000, deadline: null },
  { id: 'j6', source: 'jobkorea', title: '백엔드 시니어', company: '쿠팡', location: '서울 송파구', work_type: 'hybrid', experience: '경력 7년 이상', salary: '1억~1억5천', salary_min: 10000, salary_max: 15000, deadline: nextWeek },
];
...
for (const j of jobs) {
  sqlRun(`INSERT INTO jobs (id, source, title, company, location, work_type, experience, salary, salary_min, salary_max, deadline)
    VALUES ('${j.id}', '${j.source}', '${j.title}', '${j.company}', '${j.location}', '${j.work_type}', '${j.experience}', ${j.salary ? `'${j.salary}'` : 'NULL'}, ${j.salary_min || 'NULL'}, ${j.salary_max || 'NULL'}, ${j.deadline ? `'${j.deadline}'` : 'NULL'})`);
}
